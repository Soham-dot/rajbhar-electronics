...
export default function CartSidebar({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-card dark:bg-gray-800/60 border border-border rounded-2xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-border flex items-center justify-between">
          ...
          {itemCount > 0 && (
            <span className="bg-blue-accent text-white text-xs font-bold px-2 py-0.5 rounded-full">{itemCount}</span>
          )}
        </div>
                ...
                <div key={`${item.serviceId}-${item.issueId}`} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{item.serviceName}</p>
                      <p className="text-xs text-blue-accent mt-0.5">{item.issueName}</p>
                    </div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white shrink-0">Rs{item.price * item.quantity}</p>
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <button onClick={() => onRemove(item.serviceId, item.issueId)} className="p-1.5 text-gray-400 hover:text-red-500 transition-colors">
                ...
                <div className="flex items-center justify-between bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-600 dark:text-emerald-500">-Rs{discount} off</p>
                    </div>
                  </div>
                ...
                <div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="bg-blue-accent hover:bg-blue-accent/90 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all duration-200"
                    >
                      Apply
                    </button>
                  </div>
            ...
            <div className="p-4 bg-blue-accent">
              {discount > 0 && (
                <div className="flex justify-between text-white/70 text-xs mb-1">
                  <span>Subtotal</span>
                  <span className="line-through">Rs{total}</span>
                </div>
              )}
              {discount > 0 && (
              ...
              <button
                onClick={onCheckout}
                className="w-full flex items-center justify-between text-white transition-all duration-200"
              >
                <span className="text-lg font-bold">Rs{finalTotal}</span>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <a href="/rate-card" className="block text-xs text-blue-accent font-semibold underline hover:text-blue-accent/80 transition-colors">
                  View rate card
                </a>
              </div>
            </div>
